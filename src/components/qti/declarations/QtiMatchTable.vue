...
import QtiMatchTableEntry from '@/components/qti/declarations/QtiMatchTableEntry'

Vue.component('qti-match-table-entry', QtiMatchTableEntry)
    ...
    validateChildren () {
      ...
      this.$slots.default.forEach((slot) => {
        if (this.isValidSlot(slot)) {
          // Detect anything other than qti-match-table-entry
          if (slot.componentOptions.tag === 'qti-match-table-entry') {
            countEntries += 1
          } else {
            throw new QtiValidationException('Node is not a qti-match-table-entry: "' + slot.componentOptions.tag + '"')
          }
        }
      })
      if (countEntries < 1) {
