<template>
  <div ref="root" class="qti-feedback-block hidden">
    <slot></slot>
  </div>
</template>
...
<script>
/*
 * This class is used to define the feedback content that can be presented to the learner.
 * A feedback element that forms part of a Non-adaptive Item must not contain an interaction object,
 * either directly or indirectly. When an interaction is contained in a hidden feedback element it
 * must also be hidden. The candidate must not be able to set or update the value of the associated
 * response variables. Feedback elements can be embedded inside each other, with one
 * exception: qti-feedback-inline cannot contain qti-feedback-block elements.
 */
...
export default {
  ...
  methods: {
    ...
    evaluate () {
      ...
      console.log('[FeedbackBlock][' + this.identifier + '][evaluate]', this.outcomeIdentifier, this.outcomeDeclaration.value)
  ...
  mounted () {
    if (this.isQtiValid) {
      try {
        ...
        console.log('[FeedbackBlock]', this)
      } catch (err) {
        this.isQtiValid = false
        throw new QtiValidationException(err.message)
      }
