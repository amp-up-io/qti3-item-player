...
<script>
/*
 * The subtract operator takes 2 sub-expressions which all have single cardinality and numerical
 * base-types. The result is a single float or, if both sub-expressions are of integer type, a
 * single integer that corresponds to the first value minus the second.
    ...
    evaluate () {
      try {
        ...
        if (qtiProcessing.isNullValue(firstValue) || qtiProcessing.isNullValue(secondValue)) {
          ...
          console.log('[Subtract][Value]', this.getValue())
        ...
        this.setValue(firstValue.minus(secondValue).toNumber())
        console.log('[Subtract][Value]', this.getValue())
        return this.getValue()
      } catch (err) {
