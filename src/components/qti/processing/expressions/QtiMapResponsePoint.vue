<template>
  <div class="qti-map-response-point">
    <slot></slot>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    evaluate () {
      try {
        const responseDeclaration = qtiAttributeValidation.validateVariableIdentifierAttribute(store, this.identifier)
        ...
        if (responseDeclaration.areaMapping === null) {
          throw new QtiEvaluationException('Cannot find qti-area-mapping for response declaration: "' + this.identifier + '"')
        }

        this.setValue(qtiProcessing.mapResponsePoint(responseDeclaration))
  ...
  created () {
    try {
      ...
      if (responseDeclaration.baseType !== 'point') {
        throw new QtiValidationException(`Variable with identifier "${responseDeclaration.identifier}" must be base-type of "point".`)
      }
    } catch (err) {
      this.isQtiValid = false
      if (err.name === 'QtiValidationException') {
