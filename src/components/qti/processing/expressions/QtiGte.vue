 ...
 * if the first expression is numerically greater than or equal to the second and false
 * if it is less than the second.
...
export default {
  ...
  methods: {
    ...
    /**
     * @description Implements the "greater than or equal to" expression.  Returns true if
     * the firstValue is greater than or equal to the secondValue.  Returns false otherwise.
     * @param - firstValue, secondValue are BigNumber's
     * @return - boolean
	   */
    isGreaterThanOrEqualTo (firstValue, secondValue) {
    ...
    evaluate () {
      try {
        ...
        if (qtiProcessing.isNullValue(firstValue) || qtiProcessing.isNullValue(secondValue)) {
          ...
          console.log('[GTE][Exp1][Exp2][Value]', (firstValue === null ? null : firstValue.valueOf()), (secondValue === null ? null : secondValue.valueOf()), this.getValue())
          return this.getValue()
        }

        firstValue = new BigNumber(firstValue)
        secondValue = new BigNumber(secondValue)
        this.setValue(this.isGreaterThanOrEqualTo(firstValue, secondValue))

        console.log('[GTE][Exp1][Exp2][Value]', firstValue.valueOf(), secondValue.valueOf(), this.getValue())
        return this.getValue()
      } catch (err) {
